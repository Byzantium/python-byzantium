...
class ConfigModel:
    ...
    def _get_config_dir(self, dirname):
        '''
        ...
        '''
        confs = []
        self.logger.debug('_get_config_dir', dirname, self.ext)
        if not os.path.isdir(dirname):
            byz_dir = self.const.get('byzantium_dir')
            dirname = os.path.join(byz_dir, dirname)
        self.logger.debug(dirname+'/*.'+self.ext)
        for f in glob.glob(dirname+'/*.'+self.ext):
    ...
    def _get_config(self):
        ...
        if os.path.exists(self.config_file):
            ...
            self._values = self.load(self.config_file, 'main')
