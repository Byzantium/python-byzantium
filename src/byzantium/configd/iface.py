...
class MeshIface(IFace):
    ...
    def configure(self, max_tries=defaults.get("configd", "max-tries", set_type=int, default=15)):
        ...
        # Turn off the interface.
        self.set_down()
        """ Set wireless parameters on the interface.
        Do this by going into a loop that tests the configuration for correctness and starts the procedure over if it didn't take the first time.  We wait a few seconds between iwconfig operations because some wireless chipsets are pokey (coughAtheroscough) and silently reset themselves if you try to configure them too rapidly, meaning that they drop out of ad-hoc mode.
        """
    ...
    def add_commotion_route(self):
        ...
        return route('add', '-net', defaults.get("commotion", "ipv4_network"), 'netmask', defaults.get("commotion", "ipv4_netmask"), 'dev', self.device)
...
class ClientIFace(IFace):
    ...
    def start_captive_portal(self):
        ...
        shell(script_path, '-i', self.device, '-a', self.ipv4)

    def make_client_hosts_file(self):
        ...
        prefix = '.'.join(self.ipv4.split('.')[:3])
        # Make an /etc/hosts.mesh file, which will be used by dnsmasq to resolve its
        # mesh clients.
        hosts_string = ["%s\tbyzantium.byzantium.mesh" % self.ipv4]
        for i in range(1, 255):
            client_ip = "%s.%d" % (prefix, i)
            if client_ip != self.ipv4:
